...
{
    [Generator]
    public class JsonRpcEndpointGenerator : IIncrementalGenerator
    {
        ...
        {
            // Finde alle Klassen, die Interfaces mit [JsonRpcEndpoint] implementieren
            var classesWithRpc = context.SyntaxProvider
                .CreateSyntaxProvider(
                    ...
                    transform: (ctx, _) =>
                    {
                        ...
                        // Suche alle implementierten Interfaces mit JsonRpcEndpoint-Attribut
                        foreach (var iface in classSym.AllInterfaces)
                        {
                            var endpointAttr = iface.GetAttributes()
                            ...
                            // Sammle Interface-Methoden mit Request/Notification-Attribut
                            var methods = iface.GetMembers()
                                .OfType<IMethodSymbol>()
                                .Where(m => m.GetAttributes().Any(a =>
                                    a.AttributeClass?.ToDisplayString() is JsonRpcRequestAttributeName or JsonRpcNotificationAttributeName))
                                .ToArray();
        ...
        private void GenerateSource(SourceProductionContext context, ClassEndpointInfos classInfo)
        {
            ...
            // Check for duplicate method names at compile time
            var methodNameMap = new Dictionary<string, List<(IMethodSymbol Method, string InterfaceName, string MethodName)>>();
            ...
            foreach (var info in classInfo.EndpointInfos)
            {
                ...
                foreach (var m in info.Methods)
                {
                    var attr = m.GetAttributes().First(a =>
                        a.AttributeClass?.ToDisplayString() is JsonRpcRequestAttributeName or JsonRpcNotificationAttributeName);
                    var rpcName = string.IsNullOrWhiteSpace(attr.ConstructorArguments[0].Value as string) ? m.Name : attr.ConstructorArguments[0].Value as string ?? m.Name;
            ...
            // Report diagnostics for duplicate methods
            foreach (var entry in methodNameMap)
            {
                if (entry.Value.Count > 1)
                {
                    // Get class location - where the interfaces are implemented
                    Location location = classInfo.ClassSymbol.Locations.FirstOrDefault() ?? Location.None;

                    // Create a descriptive message about all conflicts
            ...
            foreach (var m in allMethods)
            {
                ...
                foreach (var p in m.Parameters)
                {
                    ...
                    // Add 'required' modifier if no default value is provided
            ...
            // Generate a single Attach method that handles all interfaces
            ...
            foreach (var info in classInfo.EndpointInfos)
            {
                ...
                sb.AppendLine($"        // Registering handlers for {info.InterfaceSymbol.Name}");
                sb.AppendLine($"        var target{info.InterfaceSymbol.Name} = ({interfaceName})impl;");

                // Handler-Registrierung for this interface
                foreach (var m in info.Methods)
                {
                    var attr = m.GetAttributes().First(a =>
                        a.AttributeClass?.ToDisplayString() is JsonRpcRequestAttributeName or JsonRpcNotificationAttributeName);
                    ...
                    sb.AppendLine($"            var dto = elem.Deserialize<{m.ContainingType.Name}_{m.Name}Params>(options)!;");
                    var args = string.Join(", ", m.Parameters.Select(p => $"dto.{p.Name}"));

                    if (isRequest)
                    {
                        // Request: Rückgabe an Peer
                        if (IsTaskLike(m.ReturnType))
                            sb.AppendLine($"            return target{info.InterfaceSymbol.Name}.{m.Name}({args});");
                        else
                            sb.AppendLine($"            return Task.FromResult(target{info.InterfaceSymbol.Name}.{m.Name}({args}));");
                    }
                    else
                    {
                        // Notification: kein Ergebnis erwartet
                        if (IsTaskLike(m.ReturnType))
            ...
            context.AddSource($"{className}_JsonRpc.g.cs",
        ...
        private bool IsTaskLike(ITypeSymbol typeSymbol)
        {
            ...
            if (typeSymbol.OriginalDefinition != null)
            {
                ...
                if (genericTaskSymbol != null && SymbolEqualityComparer.Default.Equals(typeSymbol.OriginalDefinition, genericTaskSymbol))
                    return true;
            }
        ...
        {
            public EndpointInfo(INamedTypeSymbol classSymbol,
        ...
        private class MethodEqualityComparer : IEqualityComparer<IMethodSymbol>
        {
            ...
            public bool Equals(IMethodSymbol x, IMethodSymbol y)
            {
                ...
                // Also check containing interface to distinguish methods from different interfaces with same name
                if (x.Name != y.Name || x.Parameters.Length != y.Parameters.Length ||
            ...
            public int GetHashCode(IMethodSymbol obj)
            {
                ...
                // Include containing type in hash calculation
